...
from accor.model_data import MAIN_TARGET, rebuild_model_data
...
def get_final_config_payload() -> dict[str, Any]:
    ...
    return {
        **base,
        ...
        "design_dir": str(FINAL_DESIGN_DIR),
        "model_name": "xgb_final",
        "pipeline": {
            "steps": [
                "Charger modèle intermédiaire multi-output",
                "Prédire toutes les cibles sur model_data",
                "X = descriptives + pred_*",
                f"Fit XGB mono-cible sur {MAIN_TARGET}",
            ],
            "note": "Stacking enrichi : descriptives + prédictions, pas preds seules.",
        },
    }


def list_final_models() -> list[dict[str, Any]]:
    FINAL_DESIGN_DIR.mkdir(parents=True, exist_ok=True)
    out: list[dict[str, Any]] = []
    for conf_path in sorted(FINAL_DESIGN_DIR.glob("*/config.json")):
        try:
            conf = json.loads(conf_path.read_text(encoding="utf-8"))
            conf.setdefault("id", conf_path.parent.name)
...
def get_final_top_model() -> dict[str, Any] | None:
    models = list_final_models()
    return models[0] if models else None


def load_final_model(model_id: str) -> dict[str, Any]:
    path = FINAL_DESIGN_DIR / _slug(model_id) / "model.pkl"
    conf_path = FINAL_DESIGN_DIR / _slug(model_id) / "config.json"
    if not path.exists():
...
def _resolve_intermediate_id(intermediate_model_id: str | None) -> str:
    mid = (intermediate_model_id or "").strip()
    if mid:
        return _slug(mid)
    top = get_top_model()
    if top and top.get("id"):
        return _slug(str(top["id"]))
    models = list_design_models()
    if not models:
        raise FileNotFoundError(
            "Aucun modèle intermédiaire dans models/design/ — "
            "entraînez d'abord un modèle dans « Modèles intermédiaires »."
        )
    return _slug(str(models[0]["id"]))
...
def train_final_model(
    *,
    ...
    """Entraîne le modèle final stacking enrichi et retourne le résultat."""
    try:
    ...
    mid = _resolve_intermediate_id(intermediate_model_id)
    if progress_hook:
    ...
    if len(train_df) < 5:
        raise ValueError(f"Trop peu de lignes train ({len(train_df)}).")
    if len(eval_df) < 1:
    ...
    name = _slug(model_name or "xgb_final")
    bundle = {
        "model": model,
    ...
    config: dict[str, Any] = {
        ...
        "pipeline": "descriptive + pred_* stacking",
        "model_file": "model.pkl",
        "config_file": "config.json",
    }

    FINAL_DESIGN_DIR.mkdir(parents=True, exist_ok=True)
    out_dir = FINAL_DESIGN_DIR / name
    if out_dir.exists():
    ...
    FINAL_LAST_FILE.write_text(
        json.dumps(
            {"name": name, "id": name, "created_at": config["created_at"], "tier": "final"},
            indent=2,
        ),
        encoding="utf-8",
    )
