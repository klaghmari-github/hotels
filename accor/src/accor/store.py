...
from accor.schemas import DatasetSchema, get_schema
...
class DatasetStore:
    ...
    def _ensure_source_files(self, schema: DatasetSchema) -> None:
        ...
        if schema.id == JOINED_DATASET_ID:
            from accor.join_data import ensure_data_xlsx

            ensure_data_xlsx(force_rebuild=False)
        if schema.id == "model_data":
            from accor.model_data import ensure_model_data

            ensure_model_data(force=False)
        if schema.id == "proximity":
            try:
                from accor.geo_proximity import ensure_hotel_proximity_data
        ...
        if schema.id == "holidays":
            try:
                from accor.geo_holidays import ensure_hotel_holidays_data
        ...
        if schema.id == "sales_raw":
            try:
                from accor.sales_prep import ensure_raw_sales_from_archive
    ...
    def load_raw(self, schema: DatasetSchema) -> pd.DataFrame:
        ...
        if schema.id == JOINED_DATASET_ID:
            from accor.join_data import _NON_NUMERIC_COLS
        ...
        if schema.id == JOINED_DATASET_ID:
            from accor.join_data import fill_numeric_nulls
    ...
    def rebuild_joined_data(
        ...
        *,
        ...
        from accor.join_data import build_joined_dataframe, save_joined_excel
    ...
    def page_payload(
        ...
        *,
        ...
        if dataset_id == "model_data":
            from accor.model_data import load_model_data_meta
