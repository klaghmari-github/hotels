...
def test_delete_zone_closes_open_tab(tmp_path: Path):
    ...
    with client:
        ...
        assert act.json()["active_tab"] == "zone_gone"
        tabs_before = client.get("/gui/tabs").json()
        assert "zone_gone" in [t["id"] for t in tabs_before["tabs"]]

        deleted = client.delete("/gui/step/zone_gone")
        assert deleted.status_code == 200, deleted.text
        body = deleted.json()
        assert body["ok"] is True
        assert "zone_gone" in body.get("closed_tabs", [])
...
def test_delete_zone_while_on_parent_still_closes_tab(tmp_path: Path):
    """Zone selectionnee depuis main: l onglet zone ouvert en fond doit partir."""
    client, pipe = _client(tmp_path)
    with client:
        client.post(
            "/gui/steps",
        ...
        assert "zone_bg" in [
            t["id"] for t in client.get("/gui/tabs").json()["tabs"]
        ]

        deleted = client.delete("/gui/step/zone_bg")
        assert deleted.status_code == 200, deleted.text
        body = deleted.json()
        assert "zone_bg" in body.get("closed_tabs", [])
