...
def test_all_prep_joins_on_hotel_code(tmp_path: Path):
    """hotel_sales_data = jointure complète sans colonnes en double."""
    inp = tmp_path / "in"
    out = tmp_path / "out"
    inp.mkdir()
    out.mkdir()
        ...
        [
            {
                "nom_hotel": "Nice",
                "hotel_code": "H2075",
                "hotel_name": "Ibis budget Nice",  # déjà présent → ROD ne doit pas dupliquer
                "annee": 2025,
                "mois": 3,
                "nombre_ventes": 5,
            }
    ...
    pd.DataFrame(
        [
            {
                "hotel_code": "H2075",
                "hotel_name": "IGNORE_PROX_NAME",
                "plage_distance_km": 0.1,
                "commerce_fb_500m": 8,
            }
        ]
        ...
        [
            {
                ...
                "zone_scolaire": "B",
                "nb_jours_feries": 1,
                "nb_jours_vacances_scolaires": 2,
                "nb_jours_vacances_hors_feries": 1,
                "jours_feries": ["2025-03-01"],
                "jours_vacances_scolaires": ["2025-03-01", "2025-03-02"],
                "jours_vacances_hors_feries": ["2025-03-02"],
            }
        ]
    ).to_parquet(inp / "hotel_holidays_data.parquet", index=False)
    pd.DataFrame(
        [
            {
                "hotel_code": "H2075",
                "hotel_name": "IGNORE_ROD_NAME",
                "hotel_city": "Nice",
                "d_recap_foo": 1,
            }
        ]
    ...
    # Pas de doublon hotel_name (garde ventes)
    assert frame.loc[0, "hotel_name"] == "Ibis budget Nice"
    assert "hotel_name_prox" not in frame.columns
    assert "hotel_name_meteo" not in frame.columns
    assert list(frame.columns).count("hotel_name") == 1
    ...
    assert frame.loc[0, "nb_jours_feries"] == 1
    assert "2025-03-01" in frame.loc[0, "jours_feries"]
    assert frame.loc[0, "hotel_city"] == "Nice"
    assert frame.loc[0, "d_recap_foo"] == 1
