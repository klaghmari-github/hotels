...
class HotelContextBuilder:
    ...
    def _load_hotels(self) -> pd.DataFrame:
        ...
        if not path.exists():
            ...
            return self._hotels
        self._hotels = pd.read_excel(path, sheet_name=0)
        return self._hotels
    ...
    def build(self, hotel_code: str) -> HotelContext:
        ...
        if not hotels.empty and "hotel_code" in hotels.columns:
            match = hotels[
                hotels["hotel_code"].astype(str).str.strip() == code
            ]
            if not match.empty:
                row = match.iloc[0].to_dict()
                sources["identity"] = "hotel_data.xlsx"
            else:
                warnings.append(f"Code {code} absent de hotel_data — fallback model_data.")
        else:
