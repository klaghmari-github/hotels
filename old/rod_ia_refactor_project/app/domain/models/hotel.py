...
class HotelIdentity:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "HotelIdentity":
        data = data or {}
        return cls(**{k: data.get(k) for k in cls.__dataclass_fields__.keys()})

class HotelOperatingState:
    """État hôtel avec variables interdépendantes.

    Toutes les variables peuvent être manipulées. Les setters recalent les
    grandeurs dépendantes : chambres occupées, clients/jour, clients/mois,
    taux d'occupation.
    """
    def __init__(self, nb_chambres: int, taux_occupation: float, guests_per_chambre: float = 1.7, jours_mois: float = 30.5):
    ...
    def _recompute_rate_from_clients_jour(self) -> None:
        ...
        self._taux_occupation = self._clip_rate(self._clients_jour / denom) if denom else 0.0
        self._chambres_occupees = self._nb_chambres * self._taux_occupation
        self._clients_mois = self._clients_jour * self._jours_mois
    ...
    def chambres_occupees(self, value: float) -> None:
        ...
        self._taux_occupation = self._clip_rate(self._chambres_occupees / self._nb_chambres) if self._nb_chambres else 0.0
        self._clients_jour = self._chambres_occupees * self._guests_per_chambre
        self._clients_mois = self._clients_jour * self._jours_mois
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "HotelOperatingState":
        data = data or {}
        return cls(
